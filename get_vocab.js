const { Client, Databases, Query } = require('node-appwrite');
const client = new Client()
  .setEndpoint('https://sgp.cloud.appwrite.io/v1')
  .setProject('6a1d8fca002bb2a78d76')
  .setKey('standard_');
const databases = new Databases(client);
databases.listDocuments('fuckyoureading', 'vocab_entries', []).then(res => console.log(res.documents.length, 'total docs')).catch(console.error);
databases.listDocuments('fuckyoureading', 'vocab_entries', [Query.lessThanEqual('next_review', new Date().toISOString())]).then(res => console.log(res.documents.length, 'due docs')).catch(console.error);
